...
void RecommendationsProcessor::addRecommendation(const Recommendation r)
{ 
  m_recommendations[r.getTicker()].push_back(r);
}
...
RecommendationType RecommendationsProcessor::averageRecommendation(const std::string &ticker)
{
  ...
  double avg = std::accumulate(recommendations.begin(), recommendations.end(),0.0)/recommendations.size();
  return (RecommendationType) (int) (avg);
}
